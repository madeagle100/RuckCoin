...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** RUCK START */
    case OP_RUCK_ASSET              : return "OP_RUCK_ASSET";
    /** RUCK END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_RUCK_ASSET) { // OP_RUCK_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == RUCK_R) { // Check to see if RUCK starts at 27 ( this->size() < 105)
                if ((*this)[28] == RUCK_V)
                    if ((*this)[29] == RUCK_N)
                        index = 30;
            } else {
                if ((*this)[28] == RUCK_R) // Check to see if RUCK starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == RUCK_V)
                        if ((*this)[30] == RUCK_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == RUCK_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == RUCK_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == RUCK_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == RUCK_R) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_RUCK_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_RUCK_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_RUCK_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_RUCK_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_RUCK_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_RUCK_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
