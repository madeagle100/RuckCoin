...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for RUCK
    ...
    if (p_rewardSnapshot.strDistributionAsset != "RUCK") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is RUCK with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using RUCK differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "RUCK") {
        // Check amount
