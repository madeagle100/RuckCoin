...
static const std::regex RAVEN_NAMES("^RUCK$|^RAVEN$|^RAVENCOIN$|^#RUCK$|^#RAVEN$|^#RAVENCOIN$");
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(RUCK_R); // r
    vchMessage.push_back(RUCK_V); // v
    vchMessage.push_back(RUCK_N); // n
    vchMessage.push_back(RUCK_Q); // q

    vchMessage.insert(vchMessage.end(), ssAsset.begin(), ssAsset.end());
    script << OP_RUCK_ASSET << ToByteVector(vchMessage) << OP_DROP;
}
...
bool TransferAssetFromScript(const CScript& scriptPubKey, CAssetTransfer& assetTransfer, std::string& strAddress)
{
    ...
    if (AreTransferScriptsSizeDeployed()) {
        ...
        // This fixes that issue (https://github.com/RavenProject/RuckCoin/issues/752)
...
{
    // Must contain at least 3 outpoints (RUCK burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Check the first transaction and verify that the correct RUCK Amount
...
bool CreateAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewAsset> assets, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string* verifier_string)
{
    ...
    // Check to make sure the wallet has the RUCK required by the burnAmount
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::vector<std::pair<CNullAssetTxData, std::string> >* nullAssetTxData, std::vector<CNullAssetTxData>* nullGlobalRestrictionData)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any RUCK, transfering an asset requires a network fee"));
        return false;
    }
    ...
    // If assetTxData is not nullptr, the user wants to add some OP_RUCK_ASSET data transactions into the transaction
    if (nullAssetTxData) {
        std::string strError = "";
        int nAddTagCount = 0;
    ...
    // nullGlobalRestiotionData, the user wants to add OP_RUCK_ASSET OP_RUCK_ASSET OP_RUCK_ASSETS data transaction to the transaction
...
void CNullAssetTxData::ConstructGlobalRestrictionTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), ssAssetTxData.begin(), ssAssetTxData.end());
    script << OP_RUCK_ASSET << OP_RESERVED << OP_RESERVED << ToByteVector(vchMessage);
}
...
void CNullAssetTxVerifierString::ConstructTransaction(CScript &script) const
{
    ...
    std::vector<unsigned char> vchMessage;
    vchMessage.insert(vchMessage.end(), ssAssetTxData.begin(), ssAssetTxData.end());
    script << OP_RUCK_ASSET << OP_RESERVED << ToByteVector(vchMessage);
}
