...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Outputs are addresses (paired with a RUCK amount, data or object specifying an asset operation) or data.\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction's inputs are not signed, and\n"
            "it is not stored in the wallet or transmitted to the network.\n"

            "\nPaying for Asset Operations:\n"
            "  Some operations require an amount of RUCK to be sent to a burn address:\n"
            "\n"
            ...
            "         x.xxx                             (number or string, required) The RUCK amount\n"
            "           or\n"
            ...
            // TODO if we decide to remove the consensus check from issue 675 https://github.com/RavenProject/RuckCoin/issues/675
...
// TODO, if we decide to remove the consensus check https://github.com/RavenProject/RuckCoin/issues/675, remove or add the code (requires consensus change)
...
//                    }
                ...
                else if (assetKey_ == "reissue")
                {
                    ...
                    if (fHasOwnerChange && !IsValidDestinationString(owner_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, owner_change_address is not a valid RuckCoin address");
                    ...
                    if (fHasOwnerChange && !IsValidDestinationString(owner_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER,
                                           "Invalid parameter, owner_change_address is not a valid RuckCoin address");
                    ...
                    if (fHasRootChange && !IsValidDestinationString(root_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, root_change_address is not a valid RuckCoin address");
                    ...
                    for (int i = 0; i < (int)addresses.size(); i++) {
                        if (!IsValidDestinationString(addresses[i].get_str()))
                            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, supplied address is not a valid RuckCoin address");
                    }
