...
std::map<std::string, CBlock> mapRUCKKAWBlockTemplates;
...
// NOTE: Unlike wallet RPC (which use RUCK values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
//    const struct VBDeploymentInfo& segwit_info = VersionBitsDeploymentInfo[Consensus::DEPLOYMENT_SEGWIT];
        ...
        fLastTemplateSupportsSegwit != fSupportsSegwit)
    {
        ...
        pindexPrev = nullptr;
        mapRUCKKAWBlockTemplates.clear();
    ...
    if (pblock->nTime >= nKAWPOWActivationTime) {
        ...
        if (IsValidDestinationString(address)) {
            ...
            if (mapRUCKKAWBlockTemplates.count(lastheader)) {
                if (pblock->nTime - 30 < mapRUCKKAWBlockTemplates.at(lastheader).nTime) {
            ...
            result.pushKV("pprpcepoch", ethash::get_epoch_number(pblock->nHeight));
            mapRUCKKAWBlockTemplates[pblock->GetKAWPOWHeaderHash().GetHex()] = *pblock;
...
static UniValue pprpcsb(const JSONRPCRequest& request) {
    ...
    if (!mapRUCKKAWBlockTemplates.count(header_hash))
        throw JSONRPCError(RPC_INVALID_PARAMS, "Block header hash not found in block data");

    std::shared_ptr<CBlock> blockptr = std::make_shared<CBlock>();
    *blockptr = mapRUCKKAWBlockTemplates.at(header_hash);
