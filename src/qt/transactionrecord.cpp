        ...
        if(fIsSingleSign)
        {
            ...
            else //We were the ones executing the transaction
            {
                ...
                for(const CTxOut &txout : wtx.tx->vout)
                {
                    if(wallet->IsMine(txout))
                    {
                        //If we sent assets, we need to see if we were sent assets or RUCK in return
                        if(txout.scriptPubKey.IsAssetScript())
                        {
            ...
            if(fSentAssets && fRecvAssets) {
                ...
                std::string asset_qty_format = RavenUnits::formatWithCustomName(QString::fromStdString(sentType), sentAmount, 2).toUtf8().constData();
                sub.assetName = strprintf("RUCK (%s %s)", TransactionView::tr("Sold").toUtf8().constData(), asset_qty_format);
            } else if (fRecvAssets) {
                //Buy!
                //Total price paid, need to use net calculation when we executed
                ...
                sub.assetName = strprintf("RUCK (%s %s)", TransactionView::tr("Bought").toUtf8().constData(), asset_qty_format);
            } else {
