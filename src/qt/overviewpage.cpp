...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(RavenUnits::RUCK),
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("RUCK")
    updateDisplayUnit();
}
...
void OverviewPage::showAssets()
{
    if (AreAssetsDeployed()) {
        ...
        // This keeps the RUCK balance grid from expanding and looking terrible when asset balance is hidden
