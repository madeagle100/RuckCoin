...
bool parseRavenURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!RavenUnits::parse(RavenUnits::RUCK, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatRavenURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(RavenUnits::format(RavenUnits::RUCK, info.amount, false, RavenUnits::separatorNever));
        paramCount++;
    }
