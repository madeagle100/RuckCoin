...
RUCKVER=($($RAVENCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $RAVEND $RAVENCLI $RAVENTX $RAVENQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${RUCKVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${RUCKVER[1]}//g" ${MANDIR}/${cmdname}.1
done
