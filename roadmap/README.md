...
RuckCoin (RUCK) is a Proof of Work coin built on the Bitcoin UTXO model. As with other Bitcoin derivatives, RUCK coins are distributed to persons augmenting the Raven network by mining Raven.
...
The RUCK used to issue assets will be sent to a burn address, which will reduce the amount of RUCK available. 

Asset transfers require the standard RUCK transaction fees for transfer from one address to another.
...
Reward capabilities will be added to allow payment (in RUCK) to all holders of an asset. Payments of RUCK would be distributed to all asset holders pro rata. This is useful for paying dividends, dividing payments, or rewarding a group of token holders.

Example: A small software company issues an asset GAMECO that represents a share of the project. GAMECO tokens can be traded with others. Once the software company profits, those profits can be distributed to all holders of GAMECO by sending the profits (via RUCK) to all holders of GAMECO.
...
Rewards allow payment in RUCK to asset holders.

[More on rewards...](./rewards/README.md)

### Phase 4 - Unique Assets (Complete)

Once created, assets can be made unique for a cost of 5 RUCK. Only non-divisible assets can be made unique. This moves an asset to a UTXO and associates a unique identifier with the txid. From this point the asset can be moved from one address to another and can be traced back to its origin. Only the issuer of the original asset can make an asset unique.  
...
Messaging to token holders by authorized senders will be layered on top of the Phase 4 unique assets. See [KAAAWWW Protocol](https://medium.com/@tronblack/ruckcoin-kaaawww-2f72077aece) for additional information.
...
Voting will be accomplished by creating and distributing parallel tokens to token holders. These tokens can be sent to RUCK addresses to record a vote.

[More on voting...](./voting/README.md)

### Phase 7 - Compatibility Mode

Allows newly created assets to appear exactly like RUCK, LTC, or Bitcoin for easy integration into exchanges, wallets, explorers, etc.
